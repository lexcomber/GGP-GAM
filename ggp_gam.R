...
tab = data.frame(tab)
rownames(tab) = c("GGP-GAM SP $x$~0~", "GGP-GAM SP $x$~1~", "GGP-GAM SP $x$~2~", "MGWR BW $x$~0~", "MGWR BW $x$~1~", "MGWR BW $x$~2~")
...
knitr::kable(tab, digits = 0, caption = "\\label{tab:tab1}Sumamries of MGWR bandwidths (BW) and GGP-GAM spline smoothing paramters (SP).", linesep = "") 
...
knitr::kable(df, caption = "\\label{tab:tab2}The variables used to construct the GGP-GAM of Leave voting rates, for each parliamentary consituency.", linesep = "",
      booktabs = T)
...
knitr::kable(tidy(m), booktabs = T, digits = 3,
      caption = paste0("\\label{tab:tab3}The results of standard OLS regression of the Brexit leave vote ($R^{2}$ = ",r2, "; AIC = ", aic, ")."))
